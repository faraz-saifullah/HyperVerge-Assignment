...
class UsersDbConnector {
  async createNewUser(userDetails) {
    const sqlQuery = {
      text: `INSERT INTO users (name, email, phone, type) 
      values ($1, $2, $3, $4)`,
      values: [userDetails.name, userDetails.email,
        userDetails.phone, userDetails.type],
    };
  ...
  async selectAdminWithCredentials(phone, password) {
    const sqlQuery = {
      text: `SELECT * FROM users where phone = ($1) and
      password = ($2) and type = ($3)`,
