...
class TicketLogsDbConnector {
  async createNewTicketLog(ticketDetails) {
    const sqlQuery = {
      text: `INSERT INTO ticket_logs 
           (from_city, to_city, traveller_id,
            seat_number, ticket_status, trip_code)
            values ($1, $2, $3, $4, $5, $6)`,
  ...
  async declineTickets(tripCode, status) {
    const sqlQuery = {
      text: `UPDATE ticket_logs SET 
            ...
            (select current_date),
            (select current_time))
            WHERE trip_code > ($2) and ticket_status = ($3)`,
  ...
  async selectAllConfirmedTickets() {
    const sqlQuery = {
      text: `SELECT (id, trip_code, from_city, to_city,
        booking_date, traveller_id,seat_number)
        from ticket_logs WHERE ticket_status = ($1)`,
      values: ['confirmed'],
    };
    return await new DataService().executeQueryAsPromise(sqlQuery);
  }
