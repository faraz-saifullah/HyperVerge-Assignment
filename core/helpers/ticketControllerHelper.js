...
class TicketControllerHelper {
  ...
  async checkIfAdminExist(phone, password) {
    let result = await this.usersDbConnector
      .selectAdminWithCredentials(phone, password);
    return result.success;
  }

  async createUserIfNotExist(details) {
    let existingUser = await this.usersDbConnector
      .selectUserWithPhone(details.phone);
    if (!existingUser.success) {
      details.type = 'traveller';
      await this.usersDbConnector.createNewUser(details);
    }
    existingUser = await this.usersDbConnector
      .selectUserWithPhone(details.phone);
  ...
  async createTripIfNotExist(details, tripCode) {
    let tripStatus = await this.tripsDbConnector
      .selectTripWithCode(tripCode);
    let tripDetails;
  ...
  async removeBookedSeatFromTrip(ticketDetails) {
    ...
    let tripDetails = await this.tripsDbConnector
      .selectTripWithCode(ticketDetails.trip_code);
    tripDetails = tripDetails.data[0];
    tripDetails.booked_seats.splice(ticketDetails.seat_number - 1, 1);
    this.tripsDbConnector
      .updateBookedSeats(tripDetails);
  }
  ...
  findFirstAvailable(seats) {
    ...
    while (low <= high) {
      if (mid + 1 < seats.length && mid + 1 === seats[mid]
		&& seats[mid + 1] === mid + 3) {
        return mid + 2;
      }
      if (mid + 1 === seats[mid]) {
