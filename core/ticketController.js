...
class TicketController {
  ...
  async bookNewTicket(details) {
    try {
      let userId = await this.ticketControllerHelper
        .createUserIfNotExist(details);
      let tripCode = this.ticketControllerHelper
        .createTripCode(details.date, details.from, details.to);
      let seatNumber = await this.ticketControllerHelper
        .createTripIfNotExist(details, tripCode);
      if (seatNumber === -1) {
  ...
  async cancelTicket(ticketId) {
    try {
      let result = await this.ticketLogsDbConnector
        .selectTicketWithId(ticketId);
      this.ticketControllerHelper
        .removeBookedSeatFromTrip(result);
      return await this.ticketLogsDbConnector
  ...
  async cancelAllTickets(details) {
    let isAdmin = await this.ticketControllerHelper
      .checkIfAdminExist(details.phone, details.password);
    if (isAdmin) {
      let tripCode = this.ticketControllerHelper
        .createTripCode(details.date, '', '');
      await this.tripsDbConnector
        .cancelTrips(tripCode);
      return await this.ticketLogsDbConnector
        .declineTickets(tripCode, 'declined');
    }
  }

  async getAllClosedTickets() {
    return await this.ticketLogsDbConnector
      .selectAllConfirmedTickets();
  }
}
